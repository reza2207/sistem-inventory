    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4" id="main">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Retur Barang Keluar</h1>
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?= base_url();?>">Home</a></li>
            <li class="breadcrumb-item active">Retur Barang Keluar</li>
          </ol>
        </nav>
      </div>
      ...
      <div class="form-group row">
        <label class="col-sm-2 col-form-label">Cari No. Faktur</label>
        <div class="col-sm-4">
          <input type="text" class="form-control" name="no_faktur" id="search_no_faktur">
        </div>
      </div>
      <?php $attr = array('id'=>'form_retur_keluar', 'class'=>'sr-only');?>
      <?= form_open('',$attr);?>
        <div class="row ">
          <div class="col-12 col-lg-12">
            <div class="form-group row">
              ...
              <div class="col-sm-4">
                <input type="text" class="form-control" name="no_faktur" id="f_no_faktur" readonly>
              </div>
            </div>
            <div class="form-group row">
        ...
        <table class="table table-hover table-striped table-bordered"  id="table" style="font-family:'Times New Roman', Times, serif; font-size: 12px;width: 100%">
          <thead class="bg-warning">
            <tr>
              ...
              <th class="text-center align-middle">Tgl. Faktur</th>
    ...
    <script>
      ...
      $( ".datepicker" ).datepicker();
        ...
        var table = $('#table').DataTable({
          ...
          "order"       : [],
          "ajax"        :{
            "url"       : "<?= site_url('retur/get_data_retur_keluar');?>",
            "type"      : "POST",
          },
          "columns":[
            {"data": ['no']},
          ...
          "createdRow" : function(row, data, index){
              ...
              $(row).attr('data-id',data['id_retur_barang_keluar']);
            ...
            $.ajax({
              ...
              success: function(result){
                ...
                if(data.type == 'success' && data.data != null){
                  $('#f_no_faktur').val(id);
                  $('#form_retur_keluar').toggleClass('sr-only');
                  let html = '';
                  let no = 0;
        ...
        $('#table').on('click', '.btn-detail', function(e){
          ...
          $.ajax({
            type: 'POST',
            data: {id: id},
            url : "<?= base_url().'Retur/get_data_id_retur_keluar';?>",
            success: function(result){
              let data = JSON.parse(result);
              ...
              $('#d_no_faktur').text(data.no_faktur);
        ...
        $('#btn-save-pdf').on('click', function(e){
          ...
          window.open("<?= base_url().'barang/get_pdf/retur_out/';?>"+id, '_blank');
          
        })
        
        $('#form_retur_keluar').on('submit', function(e){
          e.preventDefault();
          ...
          $.ajax({
            ...
            data: $(this).serialize(),
            dataType: 'JSON',
            url: '<?= base_url()."Retur/submit_retur_keluar";?>',
            success: function(result){
              ...
              if(result.type == 'error'){
                ...
                $('#form_retur_keluar input').val('');
                $('#row-barang > div').remove();
                $('#form_retur_keluar').addClass('sr-only');
